...
public class Miner extends Unit {
    ...
    boolean hasBuiltEarlyFulfillmentCenter = false;
    ...
    public void takeTurn() throws GameActionException {
        ...
        if (hqLoc != null && locsToCheck == null) {
        ...
        checkIfStuck();
        if (secondMiner)
            proteccMode = proteccMode || (rc.getRoundNum() >= proteccRound && rc.getTeamSoup() >= 150);
        /*
        if (firstMiner)
            minerAttacc();
        */
    ...
    void tryBuildDefensiveDesignSchool(Direction dir) throws GameActionException {
        if(rc.getLocation().equals(hqLoc.add(dir).add(dir)))
            if(tryBuild(RobotType.DESIGN_SCHOOL, dir)) {
                hasBuiltDesignSchool = true;
                designSchoolLoc = rc.getLocation().add(dir);
            }
            else if (tryBuild(RobotType.DESIGN_SCHOOL, dir.rotateRight())) {
                hasBuiltDesignSchool = true;
                designSchoolLoc = rc.getLocation().add(dir.rotateRight());
            }
            else if (tryBuild(RobotType.DESIGN_SCHOOL, dir.rotateRight().rotateRight())) {
                hasBuiltDesignSchool = true;
                designSchoolLoc = rc.getLocation().add(dir.rotateRight().rotateRight());
            }
            else if (tryBuild(RobotType.DESIGN_SCHOOL, dir.rotateLeft())) {
                hasBuiltDesignSchool = true;
                designSchoolLoc = rc.getLocation().add(dir.rotateLeft());
            }
            else if (tryBuild(RobotType.DESIGN_SCHOOL, dir.rotateLeft().rotateLeft())) {
                hasBuiltDesignSchool = true;
                designSchoolLoc = rc.getLocation().add(dir.rotateLeft().rotateLeft());
            }
    }
    ...
    void minerProtecc() throws GameActionException {
        ...
        if (!hasBuiltDesignSchool) {
            ...
            // if you see an enemy landscaper, build a fulfillment center
            RobotInfo [] nearbyBots = rc.senseNearbyRobots(-1, rc.getTeam().opponent());
            boolean nearbyLandscaper = false;
            for (RobotInfo bot : nearbyBots)
                nearbyLandscaper = nearbyLandscaper || bot.type == RobotType.LANDSCAPER || bot.type == RobotType.MINER;
            if (!hasBuiltEarlyFulfillmentCenter && nearbyLandscaper)
                for (Direction dir : Util.directions)
                    if (currentLoc.add(dir).distanceSquaredTo(hqLoc) > 8)
                        hasBuiltEarlyFulfillmentCenter = hasBuiltEarlyFulfillmentCenter || tryBuild(RobotType.FULFILLMENT_CENTER, dir);
            System.out.println("Built defensive design school; now trying to build refinery");
            for (Direction dir : Util.directions) {
                MapLocation loc = currentLoc.add(dir);
                if (loc.distanceSquaredTo(hqLoc) > 8 && !loc.isAdjacentTo(designSchoolLoc))
                    hasBuiltRefinery = hasBuiltRefinery || tryBuild(RobotType.REFINERY, dir);
            }
            ...
            if (!hasBuiltEarlyFulfillmentCenter && nearbyLandscaper)
                for (Direction dir : Util.directions)
                    if (currentLoc.add(dir).distanceSquaredTo(hqLoc) > 8 || rc.canSenseLocation(currentLoc.add(dir)) 
                            && rc.senseElevation(currentLoc.add(dir)) >= 20)
                        hasBuiltEarlyFulfillmentCenter = hasBuiltEarlyFulfillmentCenter || tryBuild(RobotType.FULFILLMENT_CENTER, dir);
    ...
    void checkDir(int counter, boolean designSchool) throws GameActionException{
        ...
        if (!designSchool) loc = loc.add(dir);
        if (!(rc.onTheMap(loc))) {
    ...
    void checkLoc(int counter) throws GameActionException{
        ...
        if (!rc.getLocation().equals(loc)){
            ...
            for (RobotInfo bot : bots)
                if (bot.type == RobotType.LANDSCAPER && bot.location.distanceSquaredTo(hqLoc) == 2)
                    landscaperInPlace = true;
